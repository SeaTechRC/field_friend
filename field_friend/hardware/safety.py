    ...
    def __init__(self, robot_brain: rosys.hardware.RobotBrain, *,
        ...
        if isinstance(z_axis, TornadoHardware):
            if isinstance(y_axis, YAxisStepperHardware):
                lizard_code += f'when {z_axis.name}_knife_safety and {z_axis.name}_ref_knife_ground.level == 1 then {wheels.name}.speed(0, 0); {y_axis.name}.stop(); end\n'
            elif isinstance(y_axis, YAxisCanOpenHardware):
                lizard_code += f'when {z_axis.name}_knife_safety and {z_axis.name}_ref_knife_ground.level == 1 then {wheels.name}.speed(0, 0); {y_axis.name}_motor.set_ctrl_enable(false); end\n'
